...
$HaeLunastusPVM = "SELECT LunastettuPVM FROM `tbl_asiakas_etukupongit` WHERE Kuponkinumero = '$ETKR'";

$HaeLunastusPVM_TULOS = mysqli_query($conn, $HaeLunastusPVM);
if ($HaeLunastusPVM_TULOS->num_rows > 0) {
      // output data of each row
    while($row = $HaeLunastusPVM_TULOS->fetch_assoc()) {
        $LunastusPVM = $row['LunastettuPVM'];
        $LunastusPVMMuokattu = date("d.m.Y", strtotime($LunastusPVM));
    }
}
...
<div class="container">
    <div class="row">
      ...
      <h1 class="otsikko" style="font-size:35px;color:rgb(240, 173, 78);">HUOMIO</h1><br>
                <div class="vahvistusboksi" >
                    <h1 style="text-align:center;padding-top:15px;font-weight:bolder;">ETUKUPONKI ON LUNASTETTU </h1><BR>
                    <HR>
                    <p>
                    ARVOISA ASIAKAS!<BR><BR>
                    Antamasi etukuponki, numerolla <strong><?php echo $ETKR; ?></strong> ON JO KERTALLEEN LUNASETTU
                    </p>
                    <p>
                    LUNASTETTU: <strong><?php echo $LunastusPVMMuokattu; ?></strong>
                    </p>
...
function countDown(secs,elem) {
  ...
  "<br><br>"+"VOIT SEN JÄLKEEN KOKEILLA UUDESTAAN SYÖTTÄMÄLLÄ LUNASTAMATTOMAN ERUKUPONKINUMERON";
	if(secs < 2) {
		clearTimeout(timer);
