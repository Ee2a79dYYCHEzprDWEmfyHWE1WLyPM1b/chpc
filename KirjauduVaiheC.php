          ...
          if(mysqli_num_rows($Tulos) > 0)
            {
              while($RiviTulos = mysqli_fetch_assoc($Tulos))
                {
                  $SALASAN_HASH_C = $RiviTulos["Password_Hash"];
                  $KAYTTAJATUNNUS_C = $RiviTulos["Kayttajatunnus"];


										if($Cryptaus_Salasana = $SALASAN_HASH_C && $RiviTulos["Rooli"] == "Admin")
												{
													$_SESSION['Kayttajatunnus'] = $KayttajaTunnus;
			   									echo "<script>location='ADMIN-Näkymä/ADMIN_Index.php'</script>";
												}
												else if($Cryptaus_Salasana = $SALASAN_HASH_C && $RiviTulos["Rooli"] == "Asiakas")
												{
													$Kayttajatunnus_ID = $RiviTulos['Kayttajatunnus_ID'];
                          $käyttäjätunnus = $RiviTulos['Kayttajatunnus'];
                              
                          $sql_2 = "SELECT Kayttajatunnus,Paivays 
                          		FROM tbl_kirjautumiset_c 
                          ...
                          $insert3 = "INSERT INTO tbl_kirjautumiset_c (Kayttajatunnus_ID,Kayttajatunnus,Paivays,Vaihe_ID) 
...
<html lang="fi-FI">
<head>
  <title>CHPC- Kirjaudu vaihe C</title>
  <meta charset="utf-8">
...
{
	.container > .row > .body > form >.row input[type=username],input[type=password]
	{
		width:100%;
		padding-left: 30px;
		min-width: 120px;
  ...
  <div class="modal fade" id="myModal">
    <div class="modal-dialog modal-lg">
      ...
      <div class="col-xl body">
        <p class="otsikko">Kirjautuminen vaihe, C</p>
		  <h6 id="RespoIlmoitus">Arvoisa asiakkaamme! taataksemme parhaan käyttökokemuksen, pyydämme teitä ystävällisesti käyttämään tablet- tai tietokone laitetta</h6>
            ...
            <br>
            <button  type="submit" name="kirjauduC" id="kirjauduC" class="Muokkaa btn btn-outline-primary text" style="outline:none;">Kirjaudu</button>
            </form>
            <br>
                <p class="KirjauduTeksti" style="font-size:15px;">Jos olet unohtanut salsanan, tilaa uusi <a href ="NollaaSalasanaC.php"> tästä</a></p>
